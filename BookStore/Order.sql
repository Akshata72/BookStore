...
Begin TRY
		...
		Begin
			Begin Transaction
					...
					set @OrderPrice = (select DiscountPrice from Book where BookId = @BookId);
					set @ActualPrice = (select ActualPrice From Book where BookId = @BookId);

					If((Select Quntity From Book where BookId =@BookId)>=@BookQuantity)
					Begin
							insert into Orders
							Values(GETDATE(), @BookQuantity, @OrderPrice * @BookQuantity, @ActualPrice * @BookQuantity, @BookId, @UserId,@AddressId);

							update Book Set Quntity = Quntity - @BookQuantity where BookId = @BookId;
